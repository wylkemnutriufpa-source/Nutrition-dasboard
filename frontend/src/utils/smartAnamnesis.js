...
// ==================== BANCO DE SUBSTITUIÇÕES NUTRICIONAIS ====================
/**
 * Banco de substituições para alimentos comuns
 * Cada alimento tem 1-2 alternativas nutricionalmente similares
 */
export const FOOD_SUBSTITUTIONS = {
  // PROTEÍNAS
  'ovos mexidos': ['claras em neve', 'omelete de legumes'],
  'ovos cozidos': ['ovos pochê', 'omelete simples'],
  'omelete': ['ovos mexidos', 'crepioca com recheio'],
  'frango grelhado': ['peito de peru', 'peixe branco'],
  'peito de frango': ['filé de frango', 'frango desfiado'],
  'carne magra': ['patinho moído', 'alcatra grelhada'],
  'peixe assado': ['tilápia grelhada', 'salmão ao forno'],
  'salmão grelhado': ['atum grelhado', 'truta assada'],
  'atum': ['sardinha', 'cavalinha'],
  
  // CARBOIDRATOS
  'arroz integral': ['quinoa', 'arroz selvagem'],
  'quinoa': ['amaranto', 'arroz integral'],
  'batata doce': ['inhame', 'mandioca cozida'],
  'pão integral': ['pão de centeio', 'tapioca'],
  'tapioca': ['crepioca', 'pão de batata doce'],
  'aveia': ['granola sem açúcar', 'farelo de aveia'],
  'macarrão integral': ['espaguete de abobrinha', 'macarrão de arroz'],
  
  // LEGUMINOSAS
  'feijão': ['lentilha', 'grão de bico'],
  'lentilha': ['feijão preto', 'ervilha'],
  'grão de bico': ['feijão branco', 'lentilha'],
  
  // VEGETAIS
  'brócolis': ['couve-flor', 'aspargos'],
  'couve-flor': ['brócolis', 'abobrinha'],
  'abobrinha': ['berinjela', 'couve-flor'],
  ...
  'abacate': ['coco fresco', 'nozes'],
  
  // OLEAGINOSAS
  'castanhas': ['nozes', 'amêndoas'],
  'amêndoas': ['castanha de caju', 'avelã'],
  'nozes': ['castanha do pará', 'macadâmia'],
  
  // LATICÍNIOS
  'iogurte natural': ['iogurte grego', 'kefir'],
  'queijo branco': ['ricota', 'cottage'],
  'leite desnatado': ['leite de amêndoas', 'leite de aveia'],
  
  // GORDURAS BOAS
  'azeite': ['óleo de coco', 'óleo de abacate'],
  'pasta de amendoim': ['tahine', 'pasta de amêndoa']
};

/**
 * Encontra substituições para um alimento
 * @param {string} foodName - Nome do alimento
 * @returns {Array} - Array de substituições ou vazio
 */
export function findSubstitutions(foodName) {
  if (!foodName) return [];
  
  const normalizedName = foodName.toLowerCase().trim();
  
  // Busca exata
  if (FOOD_SUBSTITUTIONS[normalizedName]) {
    return FOOD_SUBSTITUTIONS[normalizedName];
  }
  
  // Busca parcial (contém)
  for (const [key, substitutions] of Object.entries(FOOD_SUBSTITUTIONS)) {
...
/**
 * Adiciona substituições aos alimentos de uma refeição
 * @param {Array} foods - Array de alimentos (strings)
 * @returns {Array} - Array de objetos {name, substitutions}
 */
export function addSubstitutionsToFoods(foods) {
  if (!Array.isArray(foods)) return [];
  
  return foods.map(food => {
    const foodName = typeof food === 'string' ? food : food.name || food;
    const substitutions = findSubstitutions(foodName);
