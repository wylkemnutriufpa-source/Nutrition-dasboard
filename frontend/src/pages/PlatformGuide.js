...
const PlatformGuide = () => {
  ...
  const [dbFeatures, setDbFeatures] = useState(null);
  const [featureSource, setFeatureSource] = useState('loading');

  const journey = useProfessionalJourney(profile?.id);

  useEffect(() => { trackProfessionalFeature('view_platform_guide'); }, []);

  // Carregar features do banco (fonte única)
  useEffect(() => {
    loadPlatformFeatures().then(({ features, source }) => {
  ...
  // Usar features do banco se disponíveis, senão fallback
  const activeFeatures = dbFeatures || INVENTORY_FEATURES;

  const featuresByCategory = useMemo(() => getFeaturesByCategory(activeFeatures), [activeFeatures]);
