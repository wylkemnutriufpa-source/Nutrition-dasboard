...
const FeedbacksList = () => {
  ...
  useEffect(() => {
    if (professionalId) {
      loadFeedbacks();
      loadEmergencies();
      trackProfessionalFeature('view_feedbacks');
    }
  ...
  const handleResolveEmergency = async (emergencyId) => {
    try {
      ...
      trackProfessionalFeature('respond_sos');
      loadEmergencies();
    } catch (error) {
    ...
    <Layout title="Feedbacks dos Pacientes" showBack userType="professional">
      <div className="max-w-6xl mx-auto space-y-6 pb-8">
        ...
        <div className="relative overflow-hidden rounded-3xl shadow-2xl">
          <div className="bg-gradient-to-br from-rose-500 via-pink-500 to-fuchsia-500 p-6 md:p-8 text-white relative">
            ...
            <div className="relative z-10">
              <span className="inline-block px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-bold mb-3">Central de Comunicacao</span>
              <div className="flex items-center gap-4 mb-5">
                ...
                <div>
                  ...
                  <p className="text-white/80 text-sm">Acompanhe feedbacks e emergencias dos seus pacientes</p>
                </div>
              </div>
              <div className="grid grid-cols-4 gap-3">
                {[
                  { icon: MessageCircle, value: feedbacks.length, label: 'Feedbacks' },
                  { icon: AlertCircle, value: openEmergencyCount, label: 'SOS Abertos' },
                  { icon: CheckCircle, value: emergencies.filter(e => e.status === 'resolved').length, label: 'Resolvidos' },
                  { icon: Bell, value: feedbacks.filter(f => !f.professional_response).length, label: 'Pendentes' }
                ].map((stat, i) => {
