-- =====================================================
-- SQL COMPLETO - TEMPLATES GLOBAIS + EMERGÊNCIA SOS
...
BEGIN
  -- Buscar professional_id do paciente via patient_profiles
  ...
  LOOP
    ...
    IF template.type = 'tip' THEN
      ...
      SELECT
        ...
        v_professional_id,
        template.title,
        COALESCE(template.content, template.description, ''),
        template.category,
        'medium',
        template.id,
        false,
...
BEGIN
  ...
  -- Atualizar instâncias de dicas não customizadas
...
-- Função 4: Desativar template para paciente específico
CREATE OR REPLACE FUNCTION disable_template_for_patient(
  p_template_id UUID,
  p_patient_id UUID
)
RETURNS void AS $$
...
-- ==================== PARTE 4: TRIGGER PARA AUTO-SYNC ====================

-- Trigger para sincronizar templates quando novo paciente é vinculado
CREATE OR REPLACE FUNCTION auto_sync_templates_new_patient()
RETURNS TRIGGER AS $$
BEGIN
  -- Quando um vínculo patient_profile é criado
  PERFORM sync_templates_for_patient(NEW.patient_id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trigger_auto_sync_templates ON patient_profiles;
...
-- Verificação (execute separadamente para testar)
-- SELECT * FROM professional_templates LIMIT 5;
-- SELECT * FROM checklist_tasks WHERE source_template_id IS NOT NULL LIMIT 5;
-- SELECT * FROM tips WHERE source_template_id IS NOT NULL LIMIT 5;
-- SELECT * FROM feedbacks WHERE type = 'emergency' LIMIT 5;
